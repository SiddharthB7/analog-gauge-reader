...
# This code defines the GaugeDetector class, which does the following:

# 1. Loads YOLOv8 models for gauge detection and keypoint (pose) detection.
# 2. Detects the gauge in an image and crops it to the gauge region.
# 3. Runs keypoint detection on the cropped gauge to find the center, min, max, and tip points.
# 4. Returns both pixel and normalized coordinates for these keypoints.
# 5. Provides visualization functions for both gauge detection and keypoint detection.
# 6. The process_image() method combines detection and keypoint extraction for a single image.
# 7. The test_detector() function at the bottom demonstrates usage: it loads models, processes a test image, and prints/visualizes the results.

# In summary: This file is the backbone for extracting gauge locations and dial keypoints from images using YOLOv8.
